...
__global__ void scale_softmax_kernel(float *d_values,
                                     int *dSum_mat,
                                     int *dOffsets,
                                     int nnz,
                                     int seq_len,
...
__global__ void softmax_scale_backward_kernel(float *dGradAttnScore,
                                              float *dAttnScore,
                                              float *dGradAttn,
                                              int *dSum_mat,
                                              int *dOffsets,
...
struct SddmmOperation {
  ...
  void sddmm(cusparseHandle_t handle,
      ...
      float zcx_sddmm_time;
      {
          ...
          std::vector<float> hKey(num_batches * (N * K));
          cudaMemcpy(hKey.data(), dQuery, hKey.size() * sizeof(int), cudaMemcpyDeviceToHost);

          dev::vector<float> dKey_new(num_batches * (N * K));
//          for (int batchId = 0; batchId < num_batches; ++batchId) {
//              Matrix<float> matrixB(N, K, MatrixStorageOrder::row_major, hKey.data() + batchId * (N * K));
//              matrixB.changeStorageOrder();
//              cudaMemcpy(dKey_new.data() + batchId * (N * K),
//                         matrixB.values().data(),
//                         matrixB.size() * sizeof(float),
//                         cudaMemcpyHostToDevice);
//          }
          batchedMatrixTranspose(N, K, num_batches, dKey, dKey_new.data());

          sddmm_gpu_batch(num_batches, M, N, K, nnz, dQuery, dKey_new.data(), rebell, dAttn, zcx_sddmm_time);
      }
//      std::vector<float> hAttn_zcx(num_batches * nnz);
//      cudaMemcpy(hAttn_zcx.data(), dAttn, hAttn_zcx.size() * sizeof(float), cudaMemcpyDeviceToHost);

      float cusparse_sddmm_time;
      {
          size_t bufferSize = 0;
          int lda = emb_dim;
          int ldb = seq_len;
          ...
          cusparse_sddmm_time = timeCalculator.getTime();
...
//      std::vector<float> hAttn_cusparse(num_batches * nnz);
//      cudaMemcpy(hAttn_cusparse.data(), dAttn, hAttn_cusparse.size() * sizeof(float), cudaMemcpyDeviceToHost);
//
//      if (!checkData(hAttn_zcx, hAttn_cusparse)) {
//          printf("Error!!!!!!! check error\n");
//      }

      printf("zcx_sddmm_time = %f, cusparse_sddmm_time = %f, difference = %f\n",
             zcx_sddmm_time,
             cusparse_sddmm_time,
             zcx_sddmm_time - cusparse_sddmm_time);
...
struct Attention {
  ...
  void attn_forward(float *dQuery,
                    float *dKey,
                    float *dValue,
                    float *dAttn,
                    float *dOut,
  ...
  void attn_backward(float *dQuery,
                     float *dKey,
                     float *dValue,
                     float *dAttnScore,
                     float *dGradOutput,
                     float *dGradAttnScore,
                     float *dGradAttn,
                     float *dGradQuery,
                     float *dGradKey,
                     float *dGradValue,
      ...
      // cudaEventRecord(start);
      softmax_scale_backward_kernel<<<grid_size, blockdim_size>>>(dGradAttnScore,
                                                                  dAttnScore,
                                                                  dGradAttn,
                                                                  dSum_mat,
                                                                  dOffsets,
...
extern "C" void attn_forward(Attention *attn,
                             float *hQuery,
                             float *hKey,
                             float *hValue,
                             float *hAttn,
...
extern "C" void attn_backward(Attention *attn,
                              float *hQuery,
                              float *hKey,
                              float *hValue,
                              float *hAttnScore,
                              float *hGradOutput,
                              float *hGradAttnScore,
                              float *hGradAttn,
                              float *hGradQuery,
                              float *hGradKey,
                              float *hGradValue,
