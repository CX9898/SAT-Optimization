...
__global__ void checkFragmentData() {
    ...
    constexpr
    UIN aTileSize = wmmaM * wmmaK;
    constexpr
    UIN bTileSize = wmmaK * wmmaN;
...
*matrixP) {
constexpr UIN
aTileSMEMLd = (WMMA_K * 2 + 4); // Double buffer and 4 padding
constexpr UIN
bTileSMEMLd = (WMMA_K + 4); // 4 padding

constexpr UIN
aTileSMEMSize = WMMA_M * aTileSMEMLd;
constexpr UIN
bTileSMEMSize = (WMMA_N * 4) * bTileSMEMLd;
...
__global__ void sddmm_gpu_sparse_block_block512_warpOneData_shuffle(const UIN M,
    ...
    constexpr
    UIN calculateDataPerThreadBlock = numWarpsPerBlock;
