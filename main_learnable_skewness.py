    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        if len(mat_lst) == 0 :
            ...
            for l in range(model_config["num_layers"]) :
                ...
                if len(prev_l) < model_config["num_layers"] :
                    ...
                    if abs(cur_dist[l] - prev_dist[l])/prev_dist[l] * 100 >= training_config['distance'] or prev_dist[l] == 0:
                        transition = False
        ...
        if (step + 1) % training_config["eval_frequency"] == 0:
            ...
            if task == 'lra-retrieval' and (step+1) % 1000 == 0:
                ...
                best_dev_accu, update = validation(model, mat_lst, ds_iter, total_step, training_config, model_config, checkpoint_path, step, best_dev_accu, writer, task, init_t, update)
                #print(total_time)
                model.train()
                start.record()
            else:
